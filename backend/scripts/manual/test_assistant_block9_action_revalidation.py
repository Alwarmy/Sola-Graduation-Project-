...
def register_and_login() -> tuple[dict[str, str], int]:
    ...
    headers = get_shared_headers(
        "assistant_block9_5_action_revalidation",
        full_name="SOLA Assistant Block9 5 Action Revalidation",
...
def add_queue_item(headers: dict[str, str], course_id: int) -> int:
    response = requests.post(
        ...
        headers=headers,
        json={"note": "Assistant action revalidation queue item."},
        timeout=60,
    )
    payload = assert_status(response, 201)
    assert isinstance(payload, dict)
...
def create_plan(headers: dict[str, str], queue_item_id: int) -> int:
    ...
    response = requests.post(
        ...
        headers=headers,
        json={
            "title": "Assistant Action Revalidation Plan",
            "goal": "job",
            ...
            "temporary_note": "Action revalidation verification plan.",
        },
        timeout=60,
    )
    payload = assert_status(response, 201)
...
def start_complete_skip_and_make_overdue(
    ...
    skip_response = requests.post(
        ...
        headers=headers,
        json={
            "skip_reason": "Assistant action revalidation setup.",
...
def create_conversation(headers: dict[str, str]) -> int:
    response = requests.post(
        ...
        headers=headers,
        json={"title": "Assistant block 9.5 action revalidation"},
        timeout=60,
    )
    payload = assert_status(response, 201)
    assert isinstance(payload, dict)
...
def confirm_action(headers: dict[str, str], action_run_id: int) -> requests.Response:
    return requests.post(
        f"{BASE_URL}/assistant/action-runs/{action_run_id}/confirm",
        headers=headers,
        timeout=120,
    )


def find_recovery_action(exchange: dict[str, Any]) -> dict[str, Any]:
    actions = exchange.get("suggested_actions") or []
    assert actions, "Expected suggested assistant actions."

    preferred = next(
        (
            action
            for action in actions
            if action.get("action_type") in {"apply_recommended_recovery", "apply_recovery_plan", "apply_recovery", "recovery_apply"}
        ),
        None,
    )
    return preferred or actions[0]


def apply_recovery_directly(headers: dict[str, str], plan_id: int) -> dict[str, Any]:
    current_plan = read_plan(headers, plan_id)
    recovery_preview = read_recovery_preview(headers, plan_id)

    assert recovery_preview["needs_recovery"] is True
    recovery_mode = recovery_preview["recommended_recovery_mode"] or "rebalance"
    ...
    response = requests.post(
        ...
        json={
            ...
            "recovery_note": "Direct recovery apply for assistant stale-action revalidation.",
        },
        timeout=180,
    )
    payload = assert_status(response, 200)
    assert isinstance(payload, dict)
    assert payload["schedule_revision"] > recovery_preview["schedule_revision"]
    assert payload["plan_version"] > current_plan["version"]
    return payload


def assert_failed_revalidation_response(response: requests.Response) -> dict[str, Any]:
    assert response.status_code == 409, (
        f"Expected stale assistant action confirmation failure, got {response.status_code}: {response.text}"
    )
    payload = response.json()
    assert payload["error_code"] == "conflict", f"Unexpected error payload: {payload}"

    detail = payload.get("detail", "")
    assert isinstance(detail, str)
    assert "Assistant action is no longer eligible" in detail, f"Unexpected detail payload: {payload}"
    assert any(
        reason in detail
        for reason in ("stale_plan_version", "stale_schedule_revision", "stale_version")
    ), f"Expected stale revalidation reason in detail, got payload={payload}"
    return payload
...
def main() -> None:
    ...
    recovery_action = find_recovery_action(recovery_exchange)
    action_run_id = recovery_action["action_run_id"]

    section("APPLY DIRECT RECOVERY TO MAKE ACTION STALE")
    direct_recovery_result = apply_recovery_directly(headers, plan_id)

    section("CONFIRM STALE ACTION AND EXPECT REVALIDATION FAILURE")
    stale_confirm_response = confirm_action(headers, action_run_id)
    stale_payload = assert_failed_revalidation_response(stale_confirm_response)

    section("READ ACTION RUNS")
    action_runs = get("/assistant/action-runs", headers=headers)
    assert isinstance(action_runs, list)
    target_action = next(action for action in action_runs if action["id"] == action_run_id)
    assert target_action["status"] == "failed"

    section("ASSISTANT BLOCK 9.5 ACTION REVALIDATION PASSED")
    pprint(
        {
            ...
            "direct_recovery_result": {
                "plan_version": direct_recovery_result["plan_version"],
                "schedule_revision": direct_recovery_result["schedule_revision"],
                "recovery_mode": direct_recovery_result["recovery_mode"],
            },
            "stale_confirm_status": stale_confirm_response.status_code,
            "stale_confirm_error": stale_payload,
            "final_action_status": target_action["status"],
        }
    )
