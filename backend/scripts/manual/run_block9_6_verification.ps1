...
$blockNamespace = "block9_6_$timestamp"
...
$logsDir = Join-Path $root "test_reports\block9_6_$timestamp"

New-Item -ItemType Directory -Force -Path $logsDir | Out-Null

$combinedLog = Join-Path $logsDir "00_block9_6_full_report.log"
$summaryLog = Join-Path $logsDir "00_block9_6_summary.txt"

$steps = @(
    @{
        Name = "Assistant 9.6 Contract Lifecycle"
        LogFile = "01_assistant_block9_6_contract_lifecycle.log"
        Command = "python -u -m scripts.manual.test_assistant_block9_contract_lifecycle"
        Environment = @{
            SOLA_VERIFICATION_NAMESPACE = "${blockNamespace}_contract_lifecycle"
        }
    },
    @{
        Name = "Assistant 9.6 Conversation Readiness"
        LogFile = "02_assistant_block9_6_conversation_readiness.log"
        Command = "python -u -m scripts.manual.test_assistant_block9_conversation_readiness"
        Environment = @{
            SOLA_VERIFICATION_NAMESPACE = "${blockNamespace}_conversation_readiness"
        }
    }
)
...
Write-Header -Title "BLOCK 9.6 VERIFICATION START" -TargetFile $combinedLog
Write-Header -Title "BLOCK 9.6 VERIFICATION SUMMARY" -TargetFile $summaryLog
...
if ($failedSteps.Count -eq 0) {
    Write-Header -Title "BLOCK 9.6 VERIFICATION COMPLETED SUCCESSFULLY" -TargetFile $combinedLog
    Append-Text -Text "OVERALL STATUS: PASSED" -TargetFile $summaryLog

    Write-Host ""
    Write-Host "Block 9.6 verification completed successfully."
    ...
    Write-Header -Title "BLOCK 9.6 VERIFICATION COMPLETED WITH FAILURES" -TargetFile $combinedLog
    Append-Text -Text ("OVERALL STATUS: FAILED`r`nFAILED STEPS: " + ($failedSteps -join ", ")) -TargetFile $summaryLog

    Write-Host ""
    Write-Host "Block 9.6 verification completed with failures."
    Write-Host "Logs folder: $logsDir"
    Write-Host "Combined report: $combinedLog"
    Write-Host "Summary report: $summaryLog"
