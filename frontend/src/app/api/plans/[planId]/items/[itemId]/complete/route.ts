...
import {
  ...
  learningPlanItemCompleteRequestSchema,
  toPublicPlanItemActionResult,
} from "@/lib/contracts/plan-execution";

/**
 * POST /api/plans/[planId]/items/[itemId]/complete
 *
 * CP8-dedicated handler for `POST /plans/{plan_id}/items/{item_id}/complete`.
 *
 * **Concurrency:** `expected_version` (≥1) BODY is REQUIRED.
 * Optional `actual_minutes` (1–720) BODY.
 */
export async function POST(
  ...
  const parsedInput = learningPlanItemCompleteRequestSchema.safeParse(payload);
  ...
  try {
    const result = await backendRequest<unknown>(
      `/plans/${encodeURIComponent(planIdTrim)}/items/${encodeURIComponent(itemIdTrim)}/complete`,
      {
        method: "POST",
